...
csvImport.directive('ngCsvImport', function() {
	return {
		...
		link: function(scope, element) {
            ...
            scope.ignoreInitialLines = scope.ignoreInitialLines || 0;
            scope.ignoreFinalLines = scope.ignoreFinalLines || 0;
			...
			angular.element(element[0].querySelector('.separator-input')).on('keyup', function(e) {
				if ( scope.content != null ) {
					var content = {
                        ...
                        ignoreInitialLines: scope.ignoreInitialLines,
                        ignoreFinalLines: scope.ignoreFinalLines
					};
					scope.result = scope.csvToJSON(content);
					scope.$apply();
				}
			});
            ...
            scope.csvToJSON = function(content) {
                ...
                console.log("ignoreInitialLines", content.ignoreInitialLines);
                console.log("ignoreFinalLines", content.ignoreFinalLines);

                for (i = 0; i < content.ignoreInitialLines ; i++) {
                    lines.shift();
                }
                for (i = 0; i < content.ignoreFinalLines ; i++) {
                    lines.pop();
                }
